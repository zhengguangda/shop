...
public class BaseDao {
    ...
    //需要生成mapper文件的sql语句
    private static String sql = "SELECT CONCAT(a.月份,'月') AS '月份',\n" +
            "SUM(CASE a.flowname WHEN 'zk-101' THEN a.能耗 ELSE 0 END )'zk-101',\n" +
            "SUM(CASE a.flowname WHEN 'zk-102' THEN a.能耗 ELSE 0 END )'zk-102',\n" +
            "SUM(CASE a.flowname WHEN 'zk-103' THEN a.能耗 ELSE 0 END )'zk-103',\n" +
            "SUM(CASE a.flowname WHEN 'zk-104' THEN a.能耗 ELSE 0 END )'zk-104',\n" +
            "SUM(CASE a.flowname WHEN 'zk-105' THEN a.能耗 ELSE 0 END )'zk-105',\n" +
            "SUM(CASE a.flowname WHEN 'zk-106' THEN a.能耗 ELSE 0 END )'zk-106'\n" +
            "FROM\n" +
            "(SELECT MONTH(br.completetime) AS '月份',\n" +
            "bf.flowname AS flowname,\n" +
            "ec.electric*(SELECT ratio FROM `energy_standradcoal` WHERE energetype = '电')+\n" +
            "ec.water*(SELECT ratio FROM `energy_standradcoal` WHERE energetype = '水')+\n" +
            "ec.oil*(SELECT ratio FROM `energy_standradcoal` WHERE energetype = '油') AS '能耗'\n" +
            "FROM `base_flow` bf\n" +
            "LEFT JOIN `produce_report` br\n" +
            "ON br.flowid = bf.flowid\n" +
            "LEFT JOIN `energy_consume` ec\n" +
            "ON br.reportid = ec.reportid\n" +
            "GROUP BY MONTH(br.completetime),bf.flowname) a\n" +
            "GROUP BY a.月份\n" +
            "ORDER BY a.月份 ASC";
    //主表
    private static String mainTable = "energy_consume";
...
//                    插入association配置
    ...
    public   static  Connection getConn(){
        ...
        String url = "jdbc:mysql://172.25.20.123:3306/goshopping?useUnicode=true&characterEncoding=utf-8&useSSL=false&serverTimezone=UTC&zeroDateTimeBehavior=convertToNull";
